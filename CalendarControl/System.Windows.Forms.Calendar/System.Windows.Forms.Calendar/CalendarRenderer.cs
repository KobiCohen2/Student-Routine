        ...
        public GraphicsPath ItemRectangle(CalendarRendererItemBoundsEventArgs evtData, Rectangle bounds)
        {
                ...
                !evtData.Item.IsOnDayTop && evtData.Calendar.DaysMode == CalendarDaysMode.Expanded)
            {
                ...
                */
                ...
                Point b = new Point(bounds.Right, evtData.Item.MinuteStartTop);
                Point c = new Point(bounds.Right, evtData.Item.MinuteEndTop);
                Point d = new Point(bounds.Left, evtData.Item.MinuteEndTop);
                ...
                foreach (CalendarDay day in Calendar.Days)
                {
                    ...
                    foreach (List<CalendarItem> group in groups)
                    {
                        ...
                        foreach (CalendarItem item in group)
                        {
                            ...
                            item.SetMinuteEndTop(GetTimeY(item.EndDate.TimeOfDay - TimeSpan.FromMinutes(7)));
                            int rtop = item.MinuteStartTop;
                            int bottom = item.MinuteEndTop;
